...
(forceMobileSupport || !isMobile()) && // Safari has issues with timer throttling, causing low FPS when the tab goes to the background
(forceSafariSupport || !isSafari()) && typeof WebAssembly !== "undefined" && typeof OffscreenCanvas !== "undefined" && // OffscreenCanvas is required for efficient rendering
...
function buildBlurPass(gl, vertexShader, positionBuffer, texCoordBuffer, personMaskTexture, canvas, blurLevel) {
  ...
  const offset = new Array(windowSize).fill(0).map((v2, index) => index);
  const variance = sigma ** 2;
  const weights = offset.map((x2) => {
  ...
  const fragmentShaderSource = glsl`#version 300 es
    ...
    const float offset[${windowSize}] = float[](${offset.map((i2) => i2.toFixed(10)).join(", ")});
      ...
      pull: async (controller) => {
        ...
        try {
          const frame = new VideoFrame(canvas, {
            timestamp: Math.round(this.video.currentTime * 1e6)
          });
...
var BaseVideoProcessor = class {
  ...
  async start() {
    ...
    readable.pipeThrough(transformStream, { signal: this.abortController.signal }).pipeTo(writable, { signal: this.abortController.signal }).catch((e2) => {
      if (e2.name !== "AbortError" && e2.name !== "InvalidStateError") {
        console.error(`[${this.processorName}] Error processing track:`, e2);
        this.hooks.onError?.(e2);
      }
    });
...
var VirtualBackground = class extends BaseVideoProcessor {
  constructor(track, options = {}, hooks = {}) {
    super(track, hooks);
    this.options = options;
    this.segmenter = null;
    this.isSegmenterReady = false;
    this.latestCategoryMask = void 0;
    this.latestConfidenceMask = void 0;
  ...
  async initializeSegmenter() {
    try {
      ...
      const basePath = this.options.basePath || `https://unpkg.com/${packageName}@${version}/mediapipe`;
      const model = this.options.modelPath || `${basePath}/models/selfie_segmenter.tflite`;
      const fileset = await Uo.forVisionTasks(`${basePath}/wasm`);
    ...
    return new Promise((resolve) => {
      ...
      this.segmenter.segmentForVideo(frame, timestamp, (result) => {
        try {
          ...
          this.latestConfidenceMask = result.confidenceMasks?.[0]?.getAsWebGLTexture();
        } catch (err) {
...
var FullScreenBlurRenderer = class {
  constructor(canvas) {
    this.inputTexture = null;
    this.isRunning = false;
    this.targetWidth = 0;
    this.targetHeight = 0;
    this.weightCache = /* @__PURE__ */ new Map();
    this.canvas = canvas;
    const gl = canvas.getContext("webgl2", {
      alpha: false,
      antialias: false,
  ...
  createAndLinkProgram(vsSource, fsSource) {
    const gl = this.gl;
    const vs2 = this.createShader(gl.VERTEX_SHADER, vsSource);
    const fs2 = this.createShader(gl.FRAGMENT_SHADER, fsSource);
  ...
  getGaussianWeights(radius) {
    const r2 = Math.max(0, Math.min(radius | 0, 12));
    const cached = this.weightCache.get(r2);
    if (cached)
      return cached;
    const weights = new Float32Array(25);
    if (r2 === 0) {
      weights[12] = 1;
      this.weightCache.set(r2, weights);
      return weights;
    }
    const sigma = r2 * 0.6;
    let sum = 0;
    for (let i2 = -r2; i2 <= r2; i2++) {
      const w2 = Math.exp(-(i2 * i2) / (2 * sigma * sigma));
      weights[i2 + 12] = w2;
      sum += w2;
    }
    for (let i2 = -r2; i2 <= r2; i2++) {
  ...
  render(frame, radius) {
    ...
    if (scaledWidth !== this.targetWidth || scaledHeight !== this.targetHeight) {
    ...
    if (this.blurLocations.texelSizeLocation) {
      gl.uniform2f(this.blurLocations.texelSizeLocation, 1 / scaledWidth, 1 / scaledHeight);
    }
    ...
    if (this.blurLocations.directionLocation) {
      gl.uniform2f(this.blurLocations.directionLocation, 1, 0);
    }
    gl.drawArrays(gl.TRIANGLES, 0, 6);
    gl.bindFramebuffer(gl.FRAMEBUFFER, this.pongFbo);
    gl.bindTexture(gl.TEXTURE_2D, this.pingTexture);
    if (this.blurLocations.directionLocation) {
      gl.uniform2f(this.blurLocations.directionLocation, 0, 1);
    }
    gl.drawArrays(gl.TRIANGLES, 0, 6);
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    gl.viewport(0, 0, width, height);
...
var FullScreenBlur = class extends BaseVideoProcessor {
  /**
...
//# sourceMappingURL=chunk-564N6VFI.js.map
