  ...
  "node_modules/ua-parser-js/src/ua-parser.js"(exports, module) {
    (function(window3, undefined2) {
      ...
      var extend = function(regexes2, extensions) {
        ...
        return typeof version2 === STR_TYPE ? version2.replace(/[^\d\.]/g, EMPTY2).split(".")[0] : undefined2;
...
var isSupportedBrowser = async () => {
  ...
  const version2 = parseInt(major3, 10);
  return name2.includes("chrome") && version2 >= 124 || name2.includes("edge") && version2 >= 124 || name2.includes("firefox") && version2 >= 124 || name2.includes("safari") && version2 >= 17 || name2.includes("webkit") && version2 >= 605 || // WebView on iOS
  name2.includes("webview") && version2 >= 124;
};
...
var createStatsReporter = ({ subscriber, publisher, state, datacenter, pollingIntervalInMs = 2e3 }) => {
  ...
  const run = async () => {
    ...
    const subscriberResult = subscriberRawStats ? process2(subscriberRawStats, "subscriber") : { videoStats: getEmptyVideoStats(), audioStats: getEmptyAudioStats() };
    const publisherResult = publisherRawStats ? process2(publisherRawStats, "publisher") : { videoStats: getEmptyVideoStats(), audioStats: getEmptyAudioStats() };
  ...
  const streams = stats.filter((stat) => stat.type === direction && stat.kind === trackKind).map((stat) => {
    ...
    if (kind === "publisher" && publisher) {
      ...
      if (mediaSource) {
        ...
        if (trackKind === "audio" && typeof mediaSource.audioLevel === "number") {
  ...
  get trackSubscriptions() {
    ...
    for (const p of participants) {
      ...
      if (p.videoDimension && hasVideo(p)) {
        const override = videoTrackSubscriptionOverrides[p.sessionId] ?? videoTrackSubscriptionOverrides[globalOverrideKey];
  ...
  constructor(apiKeyOrArgs, opts) {
    ...
    this.shouldRejectCall = (currentCallId) => {
      if (!this.rejectCallWhenBusy)
        return false;
      return this.state.calls.some((c) => c.cid !== currentCallId && c.ringing && !c.isCreatedByMe && c.state.callingState !== CallingState.IDLE && c.state.callingState !== CallingState.LEFT && c.state.callingState !== CallingState.RECONNECTING_FAILED);
    };
...
var useModeration = (options) => {
  ...
  const processorRef = (0, import_react2.useRef)(null);
  const unregisterRef = (0, import_react2.useRef)(null);
  const blurModulePromise = (0, import_react2.useRef)(null);
  const loadVideoFiltersWebModule = (0, import_react2.useCallback)(() => {
    if (!blurModulePromise.current) {
      blurModulePromise.current = import("./index.es-P7DSX6BN.js").then((module) => module.FullScreenBlur).catch((error) => {
  ...
  const disableBlur = (0, import_react2.useCallback)(() => {
  ...
  const handleFallback = (0, import_react2.useCallback)(async () => {
    try {
      await call?.camera.disable();
    } catch (error) {
  ...
  (0, import_react2.useEffect)(() => disableBlur, [disableBlur]);
};
var MenuVisualType;
...
var useRenderer = (tfLite, call, engine) => {
  ...
  const start = (0, import_react2.useCallback)((ms, onError, onStats) => {
    let outputStream;
    let processor;
    let renderer;
...
var CallStats = (props) => {
  ...
  const [publishAudioBitrate, setPublishAudioBitrate] = (0, import_react2.useState)("-");
  const [subscribeAudioBitrate, setSubscribeAudioBitrate] = (0, import_react2.useState)("-");
...
var ParticipantOverlay = (props) => {
  ...
  const { participant } = useParticipantViewContext();
  const { useParticipantCount: useParticipantCount2, useSpeakerState: useSpeakerState2 } = useCallStateHooks();
  const participantCount = useParticipantCount2();
  const duration = useUpdateCallDuration();
  const toggleFullScreen = useToggleFullScreen();
  const { speaker, volume } = useSpeakerState2();
  const isSpeakerMuted = volume === 0;
  const { t } = useI18n();
  return (0, import_jsx_runtime2.jsx)("div", { className: "str-video__livestream-layout__overlay", children: overlayBarVisible && (0, import_jsx_runtime2.jsxs)("div", { className: "str-video__livestream-layout__overlay__bar", children: [showLiveBadge && (0, import_jsx_runtime2.jsx)("span", { className: "str-video__livestream-layout__live-badge", children: t("Live") }), showParticipantCount && (0, import_jsx_runtime2.jsx)("span", { className: "str-video__livestream-layout__viewers-count", children: humanizeParticipantCount ? humanize(participantCount) : participantCount }), showSpeakerName && (0, import_jsx_runtime2.jsx)("span", { className: "str-video__livestream-layout__speaker-name", title: participant.name || participant.userId || "", children: participant.name || participant.userId || "" }), showDuration && (0, import_jsx_runtime2.jsx)("span", { className: "str-video__livestream-layout__duration", children: formatDuration(duration) }), showMuteButton && (0, import_jsx_runtime2.jsx)("span", { className: clsx_default("str-video__livestream-layout__mute-button", isSpeakerMuted && "str-video__livestream-layout__mute-button--muted"), onClick: () => speaker.setVolume(isSpeakerMuted ? 1 : 0) }), enableFullScreen && (0, import_jsx_runtime2.jsx)("span", { className: "str-video__livestream-layout__go-fullscreen", onClick: toggleFullScreen })] }) });
};
...
var [major2, minor2, patch2] = "1.28.1".split(".");
setSdkInfo({
